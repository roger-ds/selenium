...
from settings import CEST_AUTOPECAS
...
nav.find_element(By.XPATH, '//*[@id="headerwrapper"]/nav/ul/li[7]/ul/li[6]/a').click()
nav.implicitly_wait(t)
...
for i in range(len(rows)):
    ...
    values.append(l[0])
    values.append(l2[l2.index('CEST') + 1])
    values.append(l2[l2.index('NCM') + 1])
    values.append(l2[l2.index('DESCRICAO DO PRODUTO') + 1])

    keys = ['item', 'cest', 'ncm', 'desc']
    ...
    try:
        if linha['cest'] in CEST_AUTOPECAS and select1 != '':
            button.click()
            nav.implicitly_wait(t)
            print(select1)
            print(select2)
